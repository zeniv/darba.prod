...
const PROVIDER_STYLES: Record<string, ProviderConfig> = {
  google: { label: "Google", bg: "bg-white border border-border hover:bg-gray-50", text: "text-gray-700" },
  vk: { label: "VK", bg: "bg-[#0077FF] hover:bg-[#0066DD]", text: "text-white" },
  facebook: { label: "Facebook", bg: "bg-[#1877F2] hover:bg-[#1565C0]", text: "text-white" },
  apple: { label: "Apple", bg: "bg-black hover:bg-gray-900", text: "text-white" },
  instagram: { label: "Instagram", bg: "bg-gradient-to-r from-[#833AB4] via-[#FD1D1D] to-[#F77737] hover:opacity-90", text: "text-white" },
};

const DEFAULT_STYLE: ProviderConfig = {
  label: "",
  bg: "bg-muted hover:bg-muted/80 border border-border",
        ...
        {loading ? (
          ...
          <>
            <div className="space-y-3">
              {providers.map((p) => {
                const style = PROVIDER_STYLES[p.alias] || DEFAULT_STYLE;
                const label = style.label || p.displayName;
                return (
                  <button
                    key={p.alias}
