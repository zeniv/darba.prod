...
└──────┬──────────────────┬────────────────────────┬──────────┘
       ...
       ▼                  ▼                        ▼
┌──────────────┐  ┌──────────────┐      ┌──────────────────┐
│  Next.js 14  │  │  NestJS API  │      │   Keycloak       │
...
| `auth` | JWT валидация Keycloak-токенов, сессии |
| `content` | Публикации, галерея, медиафайлы |
| `social` | Лайки, комменты, подписки, донаты |
| `payments` | Тарифы, ЮКасса, Stripe, история платежей |
| `ai-gateway` | Приём AI-запросов → запись в Redis-очередь |
| `notifications` | WebSocket уведомления, real-time события |
| `analytics` | Маркетинговый модуль, Redis Streams |
| `admin` | Управление системой, CRM, поддержка |
| `i18n` | Интернационализация бэкенда |
